...
1. **Give Codee access to the Azure DevOps project**, with the mapped work item types and
...
flowchart LR
    ...
    repo --> item[Create a story or task work item]
...
- **The mapped types and the `AI` states exist.** Codee polls the types that
  **Settings → Tasks provider → Work items** maps `story` and `task` to, which are
  `User Story` and `Task` by default, so the types are usually already in the project. The
  `AI` states are not: they come from an inherited process. If you set one up for an earlier
  project, there is nothing new to create — open **Project settings → Overview → Process**
  and point the new project at that same process. If this is your first project, follow
  [Creating states for Codee](./working-on-tasks-in-azure-devops#creating-states-for-codee)
  once and reuse the result everywhere afterwards.

  Only a project on a *different* process template needs a closer look. Check under
  [Which work items Codee polls](./working-on-tasks-in-azure-devops#which-work-items-codee-polls)
  that the types it uses are in the mapping, and add them to the row if they are not.
...
<figure className="image-block">
  <img src={require('./img/repositories_ui.png').default} alt="Repositories" />
  <figcaption>Repositories UI.</figcaption>
</figure>
...
- **Type: one the mapping selects.** A story work item (`User Story` by default) or a task
  one (`Task`). Codee ignores every other work item type.
...
Use a **story** when the work needs several steps, spans repositories, or is not yet
concrete enough to implement. Codee plans it first and you approve the plan before any code
is written.

Use a **task** when you already know what the change is and it fits in one pull request. It
...
| Work item | Start it in | What Codee does |
| --- | --- | --- |
| story | `AI Decomposition needed` | Reads the story and the code, asks any clarifying questions, creates child work items with acceptance criteria, and moves the story to `AI Decomposition review` for you to approve. |
| task | `AI Ready for development` | Implements the change on its own branch and opens a pull request, then moves the work item on to review. |
