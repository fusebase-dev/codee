...
You do not have to create anything in Jira for that. Codee has work items of its own, the
names skills declare in `x-codee-issue-type`, and that section maps each of them to the
issue types your project already uses. `story` and `task` are always there, mapped to Jira's
`Story` and `Task` by default:
...
skill will pick up either. If no list of type names covers what you need — a `Bug` that is
Codee's only when it has a label — switch the row to **JQL (Advanced)** and write the
condition yourself.
