...
To hand an issue to Codee, **move it into a status Codee watches**. What marks an issue as
the AI's is that status together with the other two filters: the issue type, through the
types mapped to Codee's work items, and a custom JQL if you set one. This tutorial uses
dedicated `AI` statuses that nothing else in your workflow touches, which is the simplest
version of that agreement. If you would rather reuse the statuses you already have, let the
work item type or the JQL be what separates Codee's issues from everyone else's.
...
You do not have to create anything in Jira for that. Codee has work items of its own, and
that section maps each of them to the issue types your project already uses. `story` and
`task` are always there, mapped to Jira's `Story` and `Task` by default:

| Codee work item | Jira issue type |
| --- | --- |
| `story` | `Story` |
| `task` | `Task` |

To change either one, click **Load types from provider**, which lists the issue types of the
configured project, and pick the right one. A row can name several types, so if your team
works bugs the same way it works tasks, map `task` to both `Task` and `Bug` and every task
skill will pick up either. If a list of type names is not enough, for example when a `Bug`
is Codee's only if it has a certain label, switch the row to **JQL (Advanced)** and write
the condition yourself.

<figure className="image-block">
  <img src={require('./img/work_items.png').default} alt="Creating a work item" />
  <figcaption>Codee's Settings for work items.</figcaption>
</figure>


**Add work item** adds a work item of your own, for example a `bug` row mapped to `Bug`. It
is worked by skills that declare `x-codee-issue-type: bug` and gets its own graph on the
Workflow page. Add a work item when it needs skills of its own; add another type to an
existing row when it does not. See [Configuration](../configuration#work-items).

Children of a mapped type are not picked up on their own: Codee works on them inside their
parent's run, so a `Task` whose parent is a `Story` — or another `Task` — is skipped on
purpose. Only the types on a row do that; a row switched to **JQL (Advanced)** does not,
because nothing in a parent says whether the condition claims it.

:::warning[The `AI` statuses have to be Codee's alone]

Any issue of a mapped type that lands in one of these statuses gets worked on. Do not reuse
an existing status your team already moves issues through, and do not add an `AI` status to
a workflow Codee should not be driving.
:::
...
<figure className="image-block">
  <img src={require('./img/jira_settings.png').default} alt="Settings for Jira connection" />
  <figcaption>Codee's Settings for Jira connection.</figcaption>
</figure>
...
In this tutorial, status is the only thing that says an issue is Codee's, and what Codee
...
Create these eight statuses. The `AI` prefix sorts them next to each other in lists and
shows on the board that the issue is Codee's.
