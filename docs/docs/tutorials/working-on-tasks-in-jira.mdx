...
Codee works in Jira like a teammate does. It has its own Jira account, looks at the issues
assigned to it, picks up the ones in a status it handles, does the work, and moves the
issue to the next status.

To hand an issue to Codee you need to do two things, and both are required:

1. **Assign the issue to Codee's Jira user.**
2. **Set the issue to an `AI` status.**
...
This tutorial shows how to create those statuses, how assignment works, and how a story
and a task move through them.

## Assigning issues to Codee

Codee logs into Jira as a dedicated account, the one you set under
**Settings → Tasks provider → Account email** (see [Configuration](../configuration)). It
only picks up issues that are in your project, assigned to that account, and in one of the
statuses its skills trigger on.

Codee keeps itself as the assignee while it works, so after you assign a story or task at
the start, you usually do not touch the assignee again until it reaches
`AI Human review`.
...
subtask that happens to be assigned to Codee and sits in a matching status is treated as
...
Create a separate Jira user for Codee instead of reusing a person's account:

- Its email is the assignee that every poll filters on.
- Its comments, status changes and pull requests are attributed to it, so you can see
  what the AI did and what a person did.
...
5. Give it a role that includes the **Assignable User**, **Transition Issues**,
   **Add Comments** and **Edit Issues** permissions. Company-managed projects list these
   under **Project settings → Permissions**.
6. Log in as Codee and create an API token at
   [id.atlassian.com/manage-profile/security/api-tokens](https://id.atlassian.com/manage-profile/security/api-tokens),
   using **Create API token**. This is the token you paste into
   **Settings → Tasks provider → API token**.

Then open any issue and check that Codee appears in the assignee dropdown. If it does not,
the account is missing from the project or lacks **Assignable User**.

## Creating statuses for Codee in Jira

### Why separate statuses

Assignment says the issue is Codee's. Status says what Codee should do with it now.
...
In every row the issue is assigned to Codee, which is what makes it visible to Codee at
...
| `AI Human review` | Codee | Codee, until a person takes it | Human does the final check and merges |
...
Reassign the issue to yourself when you pick it up. That shows who owns it and takes it
...
The whole flow is driven by the **story** issue: it stays assigned to Codee and labelled
`CodeeStory`, and its status is what triggers each run. The subtasks are worked on inside
those runs, and Codee moves their statuses itself to track where each one is.

```mermaid
flowchart TD
    start([Assign to Codee, label CodeeStory]) --> decomp[AI Decomposition needed]
...
Assign the story to Codee, add the `CodeeStory` label, and move it to
...
When all three passes are clean, the issue lands in `AI Human review`.

### 5. Human review

Read the pull request, the review comments and the QA evidence, then merge. Reassign the
issue to yourself when you pick it up, so it leaves Codee's queue and the board shows who
owns it.
...
```mermaid
flowchart TD
    start([Assign to Codee]) --> dev[AI Ready for development]
...
Assign the task to Codee and move it straight to `AI Ready for development`. There is no
planning round. Codee implements it the same way it implements a story subtask: dedicated
