...
A type that no row selects is ignored, whatever state it is in. Children of a story are not
picked up on their own either: Codee works on them inside the story's run, so a `Task` whose
parent is a mapped `User Story` is skipped on purpose.
...
is. Because their parent is a story, Codee skips them in the queue instead of treating each
as separate work.
...
6. **Is its parent a story?** Then it is skipped on purpose - its parent's run drives it.
   With the default mapping that means any `Task` under a `User Story`.
