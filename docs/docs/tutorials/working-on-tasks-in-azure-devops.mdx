...
To hand a work item to Codee, **move it into a state Codee watches**. What marks a work item
as the AI's is that state together with the other two filters: the work item type, through
the types mapped to Codee's work items, and a custom WIQL if you set one. This tutorial uses
dedicated `AI` states that nothing else in your process touches, which is the simplest
version of that agreement. If you would rather reuse the states you already have, let the
work item type or the WIQL be what separates Codee's work items from everyone else's.

You do not have to create anything in Azure DevOps for the types. Codee has two work items
of its own, `story` and `task`, and out of the box they are mapped to the standard
`User Story` and `Task`, which is what this tutorial uses. You can map them to other types,
map several types to one row, or add work items of your own. See
[Which work items Codee polls](#which-work-items-codee-polls).

:::warning[The `AI` states have to be Codee's alone]

Any work item of a mapped type that lands in one of these states gets worked on. Do not
reuse an existing state your team already moves work items through, and do not add an `AI`
state to a type Codee should not be driving.
:::

This tutorial shows how to set up the account and the app registration, how to map the work
items and create the states, and how a story and a task move through them.
...
## Which work items Codee polls

Codee has work items of its own, `story` and `task`.
**Settings → Tasks provider → Work items** is where you map each of them to the types your
organization uses. Out of the box:

| Codee work item | Azure DevOps type |
| --- | --- |
| `story` | `User Story` |
| `task` | `Task` |

If your projects use the standard Agile process, this is already correct. Leave the
mapping as it is, and the only thing you have to create is the states.

If your projects use a different process template, click **Load types from provider**, which
lists the types defined across the organization's projects, and pick the right one on each
row. On Scrum, map `story` to `Product Backlog Item`; on Basic, map `task` to `Issue`.

One row can name several types. If your team works bugs the same way it works tasks, map
`task` to both `Task` and `Bug`, and every task skill will pick up either. If a list of type
names is not enough, for example when a `Bug` is Codee's only if it has a certain tag,
switch the row to **WIQL (Advanced)** and write the condition yourself.

**Add work item** adds a row of your own, for example a `bug` mapped to your `Bug` type.
Skills can then declare `x-codee-issue-type: bug`, and the Workflow page draws that work
item its own graph. You cannot remove `story` and `task`, because the skills Codee ships
with trigger on them, but you can change what they are mapped to.
[Configuration](../configuration#work-items) describes every field in that section.

A type that no row selects is ignored, whatever state it is in. Children of a mapped type
are not picked up on their own either: Codee works on them inside their parent's run, so a
`Task` whose parent is a mapped `User Story` — or a mapped `Task` — is skipped on purpose.
Only the types on a row shield their children that way; a row switched to **WIQL
(Advanced)** does not, because nothing in a parent says whether the condition claims it.

### Using a separate type for Codee

With the default mapping, any `User Story` or `Task` in the organization is picked up once
it lands in an `AI` state. That is usually what you want: the state is the handover, so the
same backlog item is worked by a person or by Codee depending on where it sits.

If you would rather separate Codee's work by type as well, add your own types to the
inherited process that the `AI` states need anyway (see
[Create an inherited process](#create-an-inherited-process)):

1. In the inherited process, use **New work item type** to add them, for example
   `Codee Story` and `Codee Task`.
2. Add the **Priority** field to both if it is missing; it orders Codee's queue.
3. Open **Backlog levels** and put the story type in the requirement backlog and the task
   type in the iteration backlog. Until you do, the task cannot be given the story as a
   parent.
4. In Settings, click **Load types from provider** and point `story` and `task` at them.

Everything else below works the same way, only the type names change. A tag plus a custom
WIQL condition gives you the same separation without new types, so it is worth trying first.

## Creating states for Codee

### Why separate states

Type says the work item is one Codee handles at all. In this tutorial, state is the only
thing that says the work item is Codee's now, and what Codee should do with it.

Codee polls for work items whose `System.State` matches a skill's `x-codee-issue-status`
trigger. The `AI` prefix sorts Codee's stages next to each other and shows on the board that
the work item is Codee's.

### Create an inherited process

Azure DevOps does not let you add states to its own Agile, Scrum or Basic process, so the
states below need an inherited process. You create it once for the organization and apply it
to every project Codee works in:

1. **Organization settings → Boards → Process**. Open the `…` menu on the process your
   projects use and choose **Create inherited process**.
2. Point each project at it under **Projects → Change process**.

This does not change the types. An inherited Agile process still has `User Story` and
`Task`, with the same fields and boards. You are only adding states to them.
...
Create these eight states on **every type you mapped** — `User Story` and `Task` unless you
changed the mapping. In the process editor: open the type, go to **States**, and use
**New state** for each one. Every state you add must be put in a **state category** - that
is what tells Azure DevOps whether a work item counts as open, active or closed for boards,
queries and burndown.
...
children add commits to the pull request the first one opened. A task work item with no
...
work item of whatever type `task` is mapped to (`Task` by default) in any project it can
write to, and then move it to a `Done`, `Closed` or `Removed` state so it does not stay
open. It uses the mapped type instead of a fixed name, so the check uses the same type Codee
actually polls for.
...
The whole flow is driven by the **story** work item: it is of a type mapped to `story` —
`User Story` out of the box — and its state is what triggers each run. The children are
worked on inside those runs, and Codee moves their states itself to track where each one
is. Because their parent is of a mapped type, Codee skips them in the queue instead of
treating each as separate work.

```mermaid
flowchart TD
    start([Create a story work item]) --> decomp[AI Decomposition needed]
...
Create the story as a type mapped to `story` and move it to `AI Decomposition needed`. On
the next poll Codee:
...
2. Asks clarifying questions in a comment when a requirement is unclear or missing, instead
   of guessing.
3. Creates the child work items, each with a technical description, acceptance criteria
...
chore. It is a task work item with no parent, and it skips planning:

```mermaid
flowchart TD
    start([Create a task work item]) --> dev[AI Ready for development]
...
Create it as a type mapped to `task` and move it straight to `AI Ready for development`.
There is no planning round. Codee implements it the same way it implements a story child:
dedicated branch, pull request, work item comment. The review, security and QA stages work
the same as in the story flow.

### Choosing between a story and a task

Use a **story** when the work needs several children, touches more than one part of the
...
1. **Is the type mapped?** Only the types listed under
   **Settings → Tasks provider → Work items** are polled. If your `story` row still says
   `User Story`, a `Product Backlog Item` is invisible to Codee whatever its state. Map the
   row to it, or add it as a second type on the row.
...
3. **Does a skill cover this work item *and* this state?** A skill has to match both. A
   state defined on the type mapped to `task`, with only a `story` skill triggering on it,
   never fires.
...
5. **Is a custom WIQL filtering it out?** Anything in
   **Settings → Tasks provider → Custom WIQL** is `AND`ed onto every poll and can only
   narrow it. The same goes for a **WIQL (Advanced)** condition on the work item's own row.
   Empty them to rule them out.
6. **Is its parent of a mapped type?** Then it is skipped on purpose - its parent's run
   drives it. With the default mapping that means any work item under a `User Story` or a
   `Task`. A parent selected by a **WIQL (Advanced)** row does not count.
