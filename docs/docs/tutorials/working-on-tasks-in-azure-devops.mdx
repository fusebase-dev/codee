...
item has to be of a type you listed under **Settings → Tasks provider → Work items**. Codee
ignores every other type, even when it is assigned to its account and sits in a matching
state.

This tutorial uses two custom types, `Codee Story` and `Codee Task`, so that work handed to
Codee is separated from your team's own backlog by type as well as by assignment. That is a
choice, not a requirement — the mapping ships pointing at `User Story` and `Task`, and you
can leave it there and separate the work by assignment and state alone.
