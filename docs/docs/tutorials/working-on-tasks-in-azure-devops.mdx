...
There is one more requirement: the work item has to be of a type you listed under
**Settings → Tasks provider → Work items**. Codee ignores every other type, even in a
matching state.

You do not have to create anything in Azure DevOps for that. Codee has two work items of
its own, `story` and `task`, and out of the box they are mapped to the standard `User Story`
and `Task`, which is what this tutorial uses. You can map them to other types, map several
types to one row, or add work items of your own — see
...
Skills trigger on Codee's own work item names (`x-codee-issue-type: story`,
`x-codee-issue-type: task`). **Settings → Tasks provider → Work items** is where you map
each of those to the types your organization uses. Out of the box:
...
`task` to both `Task` and `Bug`, and every task skill will pick up either. If no list of
type names covers what you need — a `Bug` that is Codee's only when it is tagged — switch
the row to **WIQL (Advanced)** and write the condition yourself.
