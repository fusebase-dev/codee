...
Codee works in Azure DevOps like a teammate does. It has its own account, looks at the work
items assigned to it, picks up the ones in a state it handles, does the work, and moves the
work item to the next state.

To hand a work item to Codee you need to do two things, and both are required:

1. **Assign the work item to Codee's Azure DevOps account.**
2. **Set the work item to an `AI` state.**

And there is a third requirement, which is what the setup below is mostly about: the work
item has to be of a **custom work item type**, `Codee Task` or `Codee Story`. Codee ignores
every other type, even when it is assigned to its account and sits in a matching state.
...
- **Reading - the Entra ID app registration.** This is what you set up in
  **Settings → Tasks provider** and finish by clicking **Connect to Azure DevOps**.
  ...
  sign in with during that consent step is the one the queue filters on -the query is
  literally `[System.AssignedTo] = @Me`, and `@Me` is whoever authorized.
...
account on the agent machine. If they differ, work items are picked up as one identity and
...
- It is the assignee that every poll filters on.
- Its comments, state changes and pull requests are attributed to it, so you can see what
  the AI did and what a person did.
- If you reuse a person's account, Codee picks up everything that person is assigned.
...
3. Add it to every project it should work in, as a member of the **Contributors** group.
   Contributors can be assigned work items, change their state, comment, and create pull
   requests, which is the whole set of things Codee does.
4. On the agent machine, sign the Azure CLI in as that account: `az login`. This is the
   identity behind everything Codee writes.

Then open any work item and check that Codee appears in the **Assigned To** picker.

## Creating the work item types

### Which types Codee polls

Codee picks up exactly two work item types, and each maps to an issue type your skills
trigger on:
...
Everything else assigned to Codee is ignored, whatever state it is in. The children of a
`Codee Story` are not picked up on their own either. Codee works on them inside the story's
run.
...
Assignment and type say the work item is Codee's. State says what Codee should do with it
now.
...
In every row the work item is assigned to Codee, which is what makes it visible to Codee at
all. The `Assignee` column says who acts next.
...
| `AI Human review` | Resolved | Codee | Codee, until a person takes it | Human does the final check and merges |
...
Reassign the work item to yourself when you pick it up. That shows who owns it and takes it
out of Codee's queue for good.
...
account, same states your issue-triggered skills declare. It reports either the work items
it found or the reason Azure DevOps refused.

The MCP check goes further and exercises the write path: it asks the agent to create a
`Codee Task` in any project it can write to, assigned to the connected account, and then
move it to a `Done`, `Closed` or `Removed` state so it does not stay open.
...
The whole flow is driven by the **story** work item: it stays assigned to Codee, is of type
`Codee Story`, and its state is what triggers each run. The children are worked on inside
...
```mermaid
flowchart TD
    start([Assign to Codee, type Codee Story]) --> decomp[AI Decomposition needed]
    ...
    sec --> qa[AI Ready for QA]
    qa -->|story-qa| done([AI Human review])
...
Create the story as a `Codee Story`, assign it to Codee, and move it to
`AI Decomposition needed`. On the next poll Codee:
...
gets its own. This works against Azure Repos and against GitHub or GitLab repositories
attached to the same project - the pull request lives wherever the code does, not
necessarily in Azure DevOps.
...
When all three passes are clean, the work item lands in `AI Human review`.

### 5. Human review

Read the pull request, the review comments and the QA evidence, then merge. Reassign the
work item to yourself when you pick it up, so it leaves Codee's queue and the board shows
who owns it.
...
```mermaid
flowchart TD
    start([Assign to Codee, type Codee Task]) --> dev[AI Ready for development]
    dev -->|task-developer| cr[AI Ready for CR]
    cr -->|story-code-reviewer| sec[AI Ready for security review]
    sec --> qa[AI Ready for QA]
    qa -->|task-qa| done([AI Human review])
    cr -.->|changes needed| dev
    sec -.-> dev
    qa -.-> dev
...
Create it as a `Codee Task`, assign it to Codee and move it straight to
`AI Ready for development`. There is no planning round. Codee implements it the same way it
...
1. **Is the type right?** Only `Codee Task` and `Codee Story` are polled. A `User Story`,
   `Product Backlog Item` or `Bug` assigned to Codee is invisible to it, whatever its state.
2. **Is it assigned to the connected account?** Not to a group, and not to the account you
   happened to be signed in as when you clicked Connect. The settings page names the
   connected account.
3. **Does the state name match a skill exactly?** Case does not matter; everything else
   does. `AI Ready for dev` is not `AI Ready for development`.
4. **Does a skill cover this type *and* this state?** A skill has to match both. A state
   defined on `Codee Task` with only a `story` skill triggering on it never fires.
5. **Does the connected account have access to that project?** The query spans the
