...
`init` looks for the `claude`, `copilot` and `codex` CLIs on your PATH:

- **One found** — it is used, and `init` says so.
- **Several found** — you pick.
- **None found** — you still pick one, and install it afterwards. The CLI is only needed
